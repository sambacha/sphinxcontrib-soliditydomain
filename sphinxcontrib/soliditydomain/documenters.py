...
class SolidityObjectDocumenter(Documenter):
    ...
    def get_sourcename(self):
        return "{}:docstring of {} {}".format(
            ...
            self.object.objtype,
            ".".join(
                filter(lambda x: x, (self.object.contract_name, self.object.name))
            ),
        )
    ...
    def add_directive_header(self):
        ...
        self.add_line(
            ".. {domain}:{directive}:: {signature}".format(
        ...
        *self.options.members*.
        ...
        if not want_all:
            ...
            if should_include_fallback:
                expr |= (SolidityObject.objtype == "function") & (
                    SolidityObject.name.is_null(True)
                )
            if should_include_constructor:
                expr |= (SolidityObject.objtype == "constructor") & (
                    SolidityObject.name.is_null(True)
                )
        ...
        for member in SolidityObject.select().where(*expressions):
            ...
            full_mname = "{file}:{contract}{name}{paramtypes}".format(
                file=member.file,
                contract=""
                if member.contract_name is None
                else member.contract_name + ".",
                name=member.name or "",
                paramtypes=""
                if member.paramtypes is None
            ...
            logger.warning(
                "{} {} could not be found via query:\n{}".format(
                    ...
                    ",\n".join(
                        ...
                        + str(expr.op)
                        + ("" if expr.rhs is None else expr.rhs)
